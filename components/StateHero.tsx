    ...
    <section className="bg-slate-950 text-white">
      <div className="page-shell grid gap-8 py-12 lg:grid-cols-[1.15fr_0.85fr] lg:items-center">
        <div>
          ...
          <h1 className="mt-4 text-4xl font-black leading-tight md:text-5xl">{state.nameZh} DMV 题库</h1>
          <p className="mt-4 max-w-2xl text-base leading-7 text-slate-200">{state.summary}</p>
          <div className="mt-6 flex flex-wrap gap-3">
            <Link href={`/${state.slug}/${isCalifornia ? 'mock-test' : 'practice'}`} className="focus-ring inline-flex items-center rounded-md bg-cyan-400 px-4 py-2.5 text-sm font-black text-slate-950 hover:bg-cyan-300">
              {isCalifornia ? '开始模拟考试' : '开始练习'}
              <ArrowRight size={16} className="ml-1.5" />
            </Link>
            {isCalifornia ? (
              <Link href={`/${state.slug}/questions`} className="focus-ring inline-flex items-center rounded-md border border-white/20 px-4 py-2.5 text-sm font-bold text-white hover:bg-white/10">
                先刷 DMV 题库
              </Link>
            ) : null}
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                {dmvTools.map((tool) => (
                  <Link key={tool.href} href={`/${state.slug}/${tool.href}`} className="rounded-md bg-white/10 px-3 py-2 text-sm font-semibold text-white hover:bg-white/15">
                    {tool.title === '中文题库' ? 'DMV 题库' : tool.title}
                  </Link>
                ))}
              </div>
            </div>
