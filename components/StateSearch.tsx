    ...
    if (isExternal) {
      return <div key={state.slug} className="card flex h-full min-h-44 flex-col overflow-hidden p-0 transition hover:-translate-y-0.5 hover:border-slate-300">
        ...
        <div className="grid grid-cols-2 gap-2 p-4 pt-0">
          <a href={href} className="focus-ring inline-flex min-h-14 items-center justify-center rounded-md border border-blue-200 bg-blue-50 px-3 py-3 hover:bg-blue-100"><span className="inline-flex items-center text-sm font-bold text-blue-700">进入中文题库<ArrowRight size={15} className="ml-1.5 shrink-0" /></span></a>
          <Link href="/ny" className="focus-ring inline-flex min-h-14 items-center justify-center rounded-md border border-blue-200 bg-sky-50 px-3 py-3 hover:bg-sky-100"><span className="inline-flex items-center text-sm font-bold text-blue-700">English / 中英练习<ArrowRight size={15} className="ml-1.5 shrink-0" /></span></Link>
        </div>
      </div>
    }
