...
import { examStorageKeys, parseSavedExam } from '@/lib/exam/exam-storage'
...
function ExamSession({ questions, stateSlug, config, allowedLanguages, defaultLanguage = 'zh' }: Props) {
  ...
  const [savedExam, setSavedExam] = useState(() => parseSavedExam(browserStorage.getItem(keys.resume), stateSlug, config, questions))
  const [modeId, setModeId] = useState<string | null>(() => hasModeChoice || savedExam ? null : config.defaultModeId)
  ...
  useEffect(() => {
    ...
    const data: SavedExamV1 = { version: 1, stateSlug, modeId: mode.id, questionIds: exam.map((q) => q.id), answers, savedAt: Date.now(), startedAt }
    browserStorage.setItem(keys.resume, JSON.stringify(data))
  }, [mode, submitted, reviewIds, exam, answers, stateSlug, keys.resume, startedAt])

  useEffect(() => { if (submitted) requestAnimationFrame(() => resultRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' })) }, [submitted])

  function clearResume() { browserStorage.removeItem(keys.resume); setSavedExam(null); setResumeQuestionIds(null) }
  ...
  function chooseMode(next: string, time: number) { clearResume(); setModeId(next); setSeed(newExamSeed()); setStartedAt(time); setFinishedAt(undefined); setAnswers({}); setSubmitted(false); setReviewIds(null); setCardOpen(false) }
  function resume() { if (!savedExam) return; setModeId(savedExam.modeId); setResumeQuestionIds(savedExam.questionIds); setAnswers(savedExam.answers); setStartedAt(savedExam.startedAt ?? Date.now()); setFinishedAt(undefined); setSavedExam(null); setSubmitted(false); setReviewIds(null) }
  ...
  function restart() { clearResume(); setSeed(newExamSeed()); setStartedAt(Date.now()); setFinishedAt(undefined); setAnswers({}); setSubmitted(false); setReviewIds(null); setCardOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }) }
  ...
  function submit() {
    ...
    browserStorage.removeItem(keys.resume)
  ...
  if (!mode) return <div className="grid gap-5">{savedExam ? <section className="card border-blue-200 bg-blue-50 p-4"><p className="font-black text-blue-950">发现未完成的模拟考试</p><p className="mt-1 text-sm text-blue-800">已完成 {Object.keys(savedExam.answers || {}).length}/{savedExam.questionIds.length} 题。</p><div className="mt-3 flex gap-2"><button onClick={resume} className="rounded-md bg-blue-700 px-4 py-2 text-sm font-black text-white">继续考试</button><button onClick={clearResume} className="rounded-md border border-blue-300 bg-white px-4 py-2 text-sm font-bold text-blue-800">放弃记录</button></div></section> : null}{languageSelector}<section className="card p-5"><h1 className="text-2xl font-black">选择模拟考试模式</h1><p className="mt-2 text-sm text-slate-600">考试界面和学习功能全站统一，各州题量、通过标准和组卷规则独立。</p></section><div className="grid gap-4 md:grid-cols-2">{config.modes.map((item) => <button key={item.id} onClick={() => chooseMode(item.id, Date.now())} className="focus-ring card p-5 text-left"><span className="text-sm font-bold text-blue-700">{item.label}</span><span className="mt-2 block text-3xl font-black">{item.size} 题</span><span className="mt-2 block text-sm text-slate-600">{item.description}</span><span className="mt-4 inline-flex rounded-md bg-blue-700 px-4 py-2 text-sm font-black text-white">开始模拟</span></button>)}</div></div>
