...
const PROFILES: Record<Mode, Profile> = {
  'adult-36': { size: 36, quotas: { rules: 16, safety: 10, signs: 6, documents: 4 } },
  'teen-46': { size: 46, quotas: { rules: 20, safety: 13, signs: 8, documents: 5 } },
}
...
export default function MockTestClient({ questions, stateSlug, storageKey }: { questions: DmvQuestion[]; stateSlug: string; storageKey: string }) {
  ...
  useEffect(() => {
    try {
      ...
      if (raw) setSavedExam(JSON.parse(raw) as SavedExam)
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(resumeKey, JSON.stringify(data))
    setSavedExam(data)
  }, [mode, answers, submitted, reviewIds, testQuestions, resumeKey])

  useEffect(() => {
    if (submitted) requestAnimationFrame(() => resultRef.current?.scrollIntoView({ behavior: 'smooth', block: 'start' }))
  }, [submitted])

  function clearResume() {
    window.localStorage.removeItem(resumeKey)
    setSavedExam(null)
    setResumeQuestionIds(null)
  }
  ...
  function resume() {
    if (!savedExam) return
    setMode(savedExam.mode)
    setResumeQuestionIds(savedExam.questionIds)
  ...
  function jump(id: string) {
    ...
    setTimeout(() => setHighlighted((x) => x === id ? null : x), 1500)
  }
  ...
  function submit() {
    ...
    const ids = active.filter((q) => answers[q.id] !== q.answerIndex).map((q) => q.id)
    const existing = JSON.parse(window.localStorage.getItem(storageKey) || '[]') as string[]
    window.localStorage.setItem(storageKey, JSON.stringify(Array.from(new Set([...existing, ...ids]))))
    if (!reviewIds && language !== 'bilingual') window.localStorage.setItem(scoreKey, String(score))
  ...
  const languageSelector = (
    <section className="card p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="inline-flex items-center text-sm font-black text-slate-950"><Languages size={16} className="mr-1.5 text-blue-700" />考试语言</p>
          <p className="mt-1 text-xs leading-5 text-slate-500">
            {hasEnglishContent ? `英文内容已覆盖 ${englishCount}/${questions.length} 题；未覆盖题会自动显示中文。` : '英文内容正在校对，当前使用中文。'}
          </p>
          {language === 'bilingual' ? <p className="mt-1 text-xs font-bold text-amber-700">中英对照属于学习辅助模式，本次成绩仅作学习参考。</p> : null}
        </div>
  ...
  if (isCalifornia && !mode) return <div className="grid gap-5">
    ...
    {languageSelector}
    <section className="card p-5"><p className="text-sm font-bold text-teal-700">California DMV 模拟考试</p><h1 className="mt-2 text-2xl font-black">选择模拟考试模式</h1><p className="mt-3 text-sm leading-6 text-slate-600">36 题和 46 题为参考历史题量设计的练习模式；本站以 80% 作为练习目标，正式考试题量和要求以 California DMV 当日规定为准。</p></section>
    <div className="grid gap-4 md:grid-cols-2"><button onClick={() => start('adult-36')} className="focus-ring card p-5 text-left"><span className="text-sm font-bold text-blue-700">标准模拟 · 参考历史题量</span><span className="mt-2 block text-3xl font-black">36 题</span><span className="mt-4 inline-flex rounded-md bg-blue-700 px-4 py-2 text-sm font-black text-white">开始 36 题模拟</span></button><button onClick={() => start('teen-46')} className="focus-ring card p-5 text-left"><span className="text-sm font-bold text-violet-700">青少年强化 · 参考历史题量</span><span className="mt-2 block text-3xl font-black">46 题</span><span className="mt-4 inline-flex rounded-md bg-slate-950 px-4 py-2 text-sm font-black text-white">开始 46 题模拟</span></button></div>
  </div>
  ...
  return <div className="grid gap-5">
    ...
    {submitted ? <section ref={resultRef} className={`card scroll-mt-6 p-4 ${score >= TARGET ? 'border-green-300 bg-green-50' : 'border-amber-300 bg-amber-50'}`}><p className="text-sm font-bold text-slate-600">考试结果{language === 'bilingual' ? ' · 中英辅助' : ''}</p><p className={`mt-1 text-3xl font-black ${score >= TARGET ? 'text-green-800' : 'text-amber-900'}`}>{score >= TARGET ? '达到练习目标' : '建议继续复习'} · {score}%</p>{language === 'bilingual' ? <p className="mt-2 text-xs font-bold text-amber-800">本次使用中英对照，成绩仅作学习参考，不计入最近模拟成绩。</p> : null}<div className="mt-3 grid grid-cols-3 gap-2"><div className="rounded-md bg-white/80 p-2"><p className="text-[10px] text-slate-500">答对</p><p className="text-lg font-black">{correct}/{active.length}</p></div><div className="rounded-md bg-white/80 p-2"><p className="text-[10px] text-slate-500">错题</p><p className="text-lg font-black">{wrong.length}</p></div><div className="rounded-md bg-white/80 p-2"><p className="text-[10px] text-slate-500">目标</p><p className="text-lg font-black">80%</p></div></div><div className="mt-4 border-t border-slate-200 pt-3"><p className="text-sm font-black">分类成绩</p><div className="mt-2 overflow-hidden rounded-lg border border-slate-200 bg-white/80">{categoryResults.map((x) => <div key={x.category} className="flex items-center gap-3 border-b border-slate-100 px-3 py-2 last:border-0"><span className="min-w-20 text-sm font-bold text-slate-700">{categoryLabels[x.category]}</span><div className="h-1.5 flex-1 overflow-hidden rounded-full bg-slate-100"><div className={`h-full ${x.score < TARGET ? 'bg-amber-500' : 'bg-green-600'}`} style={{ width: `${x.score}%` }} /></div><span className="text-xs text-slate-500">{x.correct}/{x.total}</span><b className={`w-11 text-right text-sm ${x.score < TARGET ? 'text-amber-800' : 'text-green-800'}`}>{x.score}%</b></div>)}</div>{weak.length ? <p className="mt-3 text-sm font-black">优先复习：{weak.slice(0, 2).map((x) => categoryLabels[x.category]).join('、')}</p> : <p className="mt-3 text-sm font-bold text-green-800">所有分类都达到练习目标。</p>}</div><div className="mt-4 flex flex-wrap gap-2">{wrong.length ? <button onClick={reviewWrong} className="focus-ring rounded-md bg-blue-700 px-4 py-2 text-sm font-black text-white">重新练习本次 {wrong.length} 道错题</button> : null}<button onClick={restart} className="focus-ring rounded-md bg-slate-950 px-4 py-2 text-sm font-black text-white">再考一套</button><Link href={`/${stateSlug}/wrong-questions`} className="focus-ring rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700">全部错题本</Link>{isCalifornia ? <Link href="/california" className="focus-ring rounded-md border border-slate-300 bg-white px-4 py-2 text-sm font-bold text-slate-700">返回加州 DMV</Link> : null}</div></section> : null}

    <div className="grid gap-4">{active.map((q, i) => {
      const selected = answers[q.id]
      const english = getEnglishContent(q)
      const effectiveLanguage: DmvLanguage = language === 'en' && !english ? 'zh' : language
      return <article id={`question-${q.id}`} key={q.id} className={`card scroll-mt-24 p-4 transition ${highlighted === q.id ? 'border-amber-400 ring-4 ring-amber-100' : ''}`}>
        {language === 'bilingual' && english ? <div><h2 className="text-lg font-black leading-7">{i + 1}. {q.question}</h2><p className="mt-1 text-sm font-semibold leading-6 text-slate-600">{english.question}</p></div> : <h2 className="text-lg font-black leading-7">{i + 1}. {getQuestionText(q, effectiveLanguage)}</h2>}
        {language !== 'zh' && !english ? <p className="mt-2 text-xs font-bold text-amber-700">本题英文正在校对，暂时显示中文。</p> : null}
        <QuestionSignImage question={q} stateSlug={stateSlug} />
        <div className="mt-4 grid gap-2">{q.choices.map((choice, ci) => {
          const englishChoice = english?.choices[ci]
          return <button key={`${q.id}-${ci}`} disabled={submitted} onClick={() => setAnswers((a) => ({ ...a, [q.id]: ci }))} className={`focus-ring flex justify-between gap-3 rounded-md border p-3 text-left text-sm ${submitted && ci === q.answerIndex ? 'border-green-300 bg-green-50' : submitted && selected === ci && ci !== q.answerIndex ? 'border-rose-300 bg-rose-50' : selected === ci ? 'border-blue-400 bg-blue-50' : 'border-slate-200 bg-white'}`}><span>{language === 'bilingual' && englishChoice ? <><span className="block">{choice}</span><span className="mt-0.5 block text-xs text-slate-500">{englishChoice}</span></> : getChoiceText(q, ci, effectiveLanguage)}</span>{submitted && ci === q.answerIndex ? <CheckCircle2 size={18} className="shrink-0 text-green-700" /> : null}{submitted && selected === ci && ci !== q.answerIndex ? <XCircle size={18} className="shrink-0 text-rose-700" /> : null}</button>
        })}</div>
        {submitted ? language === 'bilingual' && english ? <div className="mt-3 text-sm leading-6 text-slate-600"><p>{q.explanation}</p><p className="mt-2 border-t border-slate-200 pt-2">{english.explanation}</p></div> : <p className="mt-3 text-sm leading-6 text-slate-600">{getExplanationText(q, effectiveLanguage)}</p> : null}
      </article>
    })}</div>
