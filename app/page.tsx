...
export default function HomePage() {
  return <>
    ...
    <section className="bg-white py-12"><div className="page-shell grid gap-4 lg:grid-cols-2"><div className="card p-5"><h2 className="text-2xl font-black text-slate-950">第一次考美国驾照？</h2><div className="prose-copy mt-3 text-sm leading-7 text-slate-700"><p>先选择所在州，用完整题库理解常见交通规则和标志，再通过模拟考试检查掌握程度。</p><p>各州考试题量、证件、费用和预约政策可能调整，正式申请前请使用州页面提供的官方链接确认最新要求。</p></div></div><div className="card p-5"><h2 className="text-2xl font-black text-slate-950">纽约 DMV 驾照题库</h2><p className="mt-3 text-sm leading-7 text-slate-700">纽约驾照笔试题库已在 OpenAA 主站上线，提供题库练习、模拟考试、交通标志和错题复习。</p><div className="mt-5 grid grid-cols-2 gap-3"><a href={OPENAA_DMV_URL} className="focus-ring inline-flex items-center justify-center rounded-md border border-blue-200 bg-blue-100 px-4 py-3 text-sm font-black text-blue-800 hover:bg-blue-200">进入中文题库<ArrowRight size={15} className="ml-1.5" /></a><Link href="/ny" className="focus-ring inline-flex items-center justify-center rounded-md border border-sky-200 bg-sky-50 px-4 py-3 text-sm font-black text-sky-800 hover:bg-sky-100">English / 中英练习<ArrowRight size={15} className="ml-1.5" /></Link></div></div></div></section>
    <OpenAACrossLinks />
  </>
}
