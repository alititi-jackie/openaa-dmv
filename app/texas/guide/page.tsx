...
export default function TexasGuidePage() {
  ...
  const intro = <><p>Texas DPS 当前官方资料明确：Knowledge Exam 至少达到 70% 才通过。普通非商业驾照正式知识考试目前提供 English 或 Spanish，不提供中文。</p><p>本站当前提供 {count} 道 Texas 专属题与公共核心题，支持中文、English 和中英对照。本站 30 题模式是练习模式，不把第三方常见题量说法冒充为 DPS 当前官方固定题量。</p></>
  const notice = <div className="mb-6 rounded-lg border-2 border-amber-300 bg-amber-50 p-4"><p className="font-black text-amber-900">正式考试语言提醒</p><p className="mt-1 text-sm leading-6 text-amber-900">德州普通非商业驾照 Knowledge Test 目前提供 English 或 Spanish，不提供中文；本站中文内容用于学习辅助。</p></div>
  const keywords = <div className="card p-5"><h2 className="text-xl font-black text-slate-950">考试英文关键词</h2><div className="mt-3 grid gap-2 text-sm text-slate-700"><p><b>yield</b> · 让行</p><p><b>right-of-way</b> · 路权</p><p><b>divided highway</b> · 分隔道路</p><p><b>flashing red lights</b> · 闪烁红灯</p><p><b>shoulder</b> · 路肩</p><p><b>intoxicated</b> · 醉酒/受影响</p><p><b>provisional license</b> · 阶段性驾照</p><p><b>learner license</b> · 学习驾照</p></div></div>
  return <><JsonLd data={webPageJsonLd('2026 德州 DPS 中文驾照笔试指南', 'Texas DPS Knowledge Test 中文、English 和中英对照学习指南，正式考试不提供中文，官方通过标准为70%。', '/texas/guide')} /><JsonLd data={faqJsonLd(faq)} /><JsonLd data={breadcrumbJsonLd([{ name: '首页', path: '/' }, { name: '德克萨斯州', path: '/texas' }, { name: '考试指南', path: '/texas/guide' }])} /><StateGuideLayout state={state} backLabel="返回德州 DPS" eyebrow="Texas DPS Driver Guide" title="2026 德州 DPS 中文驾照笔试指南" intro={intro} steps={steps} questionCount={count} examLabel="30 题模拟考试" notice={notice} extraAside={keywords}/><OpenAACrossLinks /></>
}
