...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const title = isNJ ? '2026 新泽西 MVC 50题模拟考试｜40题正确通过' : isMA ? '2026 麻州 RMV 25题模拟考试｜18题正确通过' : `${stateAgencyLabel(state)} 中文模拟考试`
  const description = isNJ ? '新泽西 MVC 驾照知识考试模拟练习，按 50 题、40 题正确、80% 通过标准设计。' : isMA ? 'Massachusetts RMV Class D learner’s permit 模拟考试：25题、25分钟、至少答对18题通过，支持中文、English和中英对照。' : stateDescription(state)
...
export default async function MockTestPage({ params }: Props) {
  ...
  const isNJ = state.slug === 'new-jersey'
  const isMA = state.slug === 'massachusetts'
  const title = isNJ ? '2026 新泽西 MVC 50题模拟考试' : isMA ? '2026 麻州 RMV 25题模拟考试' : `${stateAgencyLabel(state)} 中文模拟考试`
  const description = isNJ ? '按 New Jersey MVC 50题、80%通过标准设计的模拟考试。' : isMA ? '按 Massachusetts RMV Class D Permit Exam 的 25题、25分钟、18题正确通过标准设计。' : stateDescription(state)
  return <section className="bg-[#f4f7fb] py-10"><JsonLd data={webPageJsonLd(title, description, `/${state.slug}/mock-test`)} /><JsonLd data={breadcrumbJsonLd([{ name: '首页', path: '/' }, { name: state.nameZh, path: `/${state.slug}` }, { name: '模拟考试', path: `/${state.slug}/mock-test` }])} /><div className="page-shell"><BackLink href={`/${state.slug}`} label={stateBackLabel(state)} />{isMA ? <ExamTimer minutes={25} /> : null}<MockTestClient questions={questions} stateSlug={state.slug} config={config} /></div></section>
}
