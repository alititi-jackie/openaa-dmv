...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: isNJ ? '2026 新泽西 MVC 50题模拟考试｜40题正确通过' : `${state.nameZh} DMV 中文模拟考试`, description: isNJ ? '新泽西 MVC 驾照知识考试模拟练习，按官方 50 题、40 题正确、80% 通过标准设计。' : stateDescription(state), alternates: { canonical: `/${state.slug}/mock-test` } }
}
...
export default async function MockTestPage({ params }: Props) {
  ...
  return <section className="bg-[#f4f7fb] py-10"><JsonLd data={webPageJsonLd(isNJ ? '2026 新泽西 MVC 50题模拟考试' : `${state.nameZh} DMV 中文模拟考试`, isNJ ? '按 New Jersey MVC 50题、80%通过标准设计的模拟考试。' : stateDescription(state), `/${state.slug}/mock-test`)} /><JsonLd data={breadcrumbJsonLd([{ name: '首页', path: '/' }, { name: state.nameZh, path: `/${state.slug}` }, { name: '模拟考试', path: `/${state.slug}/mock-test` }])} /><div className="page-shell"><BackLink href={`/${state.slug}`} label={`返回${state.nameZh}${isNJ ? ' MVC' : ' DMV'}`} />{isNJ ? <NewJerseyMockTestClient questions={questions} storageKey="openaa-dmv:new-jersey:wrong" /> : <MockTestClient questions={questions} stateSlug={state.slug} storageKey={`openaa-dmv:${state.slug}:wrong`} />}</div></section>
}
