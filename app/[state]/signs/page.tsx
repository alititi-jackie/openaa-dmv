...
export default async function SignsPage({ params }: Props) { const { state: stateSlug } = await params; const state = getLiveStateBySlug(stateSlug); if (!state) notFound(); const isCalifornia = state.slug === 'california'; const allSignQuestions = getStateQuestionsByCategory(state.slug, 'signs'); const illustrated = allSignQuestions.filter((question) => getQuestionSignMeta(question) !== null); const signQuestions = isCalifornia ? illustrated.filter((question) => /^ca2-signs-(?:00[1-9]|01\d|020)$/.test(question.id)) : illustrated; return <section className="bg-[#f4f7fb] py-10"><JsonLd data={webPageJsonLd(isCalifornia ? '2026 加州 DMV 交通标志题库' : `${stateAgencyLabel(state)} 交通标志练习`, isCalifornia ? '加州 DMV 交通标志、信号和道路标线中英文专项题库。' : stateDescription(state), `/${state.slug}/signs`)} /><JsonLd data={breadcrumbJsonLd([{ name: '首页', path: '/' }, { name: state.nameZh, path: `/${state.slug}` }, { name: '交通标志', path: `/${state.slug}/signs` }])} /><div className="page-shell"><BackLink href={`/${state.slug}`} label={stateBackLabel(state)} /><div className="mb-6"><p className="text-sm font-bold text-teal-700">Road Signs</p><h1 className="mt-2 text-3xl font-black text-slate-950">{stateAgencyLabel(state)} 交通标志练习</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">共 {signQuestions.length} 道已配对应图片的交通标志识图题。交通标志专项只显示已有图片映射的题目。</p></div><QuestionsClient questions={signQuestions} storageKey={`openaa-dmv:${state.slug}:wrong`} stateSlug={state.slug} /></div></section> }
