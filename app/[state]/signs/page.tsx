...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: isCalifornia ? '2026 加州 DMV 交通标志题库｜中英文识图练习' : `${state.nameZh} DMV 交通标志练习`, description: isCalifornia ? '加州 DMV 交通标志专项题库，支持中文、English 和中英对照。' : stateDescription(state), alternates: { canonical: `/${state.slug}/signs` } }
}
...
export default async function SignsPage({ params }: Props) {
  ...
  return <section className="bg-[#f4f7fb] py-10"><JsonLd data={webPageJsonLd(isCalifornia ? '2026 加州 DMV 交通标志题库' : `${state.nameZh} DMV 交通标志练习`, isCalifornia ? '加州 DMV 交通标志、信号和道路标线中英文专项题库。' : stateDescription(state), `/${state.slug}/signs`)} /><JsonLd data={breadcrumbJsonLd([{ name: '首页', path: '/' }, { name: state.nameZh, path: `/${state.slug}` }, { name: '交通标志', path: `/${state.slug}/signs` }])} /><div className="page-shell"><BackLink href={`/${state.slug}`} label={`返回${state.nameZh}${isNJ ? ' MVC' : ' DMV'}`} /><div className="mb-6"><p className="text-sm font-bold text-teal-700">Road Signs</p><h1 className="mt-2 text-3xl font-black text-slate-950">{isCalifornia ? '加州 DMV 交通标志题库' : isNJ ? '新泽西 MVC 交通标志练习' : `${state.nameZh} DMV 交通标志练习`}</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-slate-600">共 {signQuestions.length} 道已配正确对应图片的交通标志识图题。交通标志专项不再显示无图题。</p></div><QuestionsClient questions={signQuestions} storageKey={`openaa-dmv:${state.slug}:wrong`} stateSlug={state.slug} /></div></section>
}
