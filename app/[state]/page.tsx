...
export default async function StatePage({ params }: Props) {
  ...
  const pageDescription = isCalifornia ? `当前提供 ${questions.length} 道中英双语练习题，覆盖道路规则、交通标志和安全驾驶，并配套模拟考试、错题本和官方 California DMV 学习入口。` : stateDescription(state)
  ...
  return <>
    ...
    {(isCalifornia || isNJ) ? <section className="bg-white py-10"><div className="page-shell grid gap-4 md:grid-cols-3">
      <div className="card p-5"><p className="text-sm font-bold text-teal-700">当前题库</p><p className="mt-2 text-3xl font-black text-slate-950">{questions.length} 题</p><p className="mt-2 text-sm leading-6 text-slate-600">{isNJ ? '新泽西专属考点 + 去重筛选后的公共核心题，支持中文、English 和中英对照。' : '加州专属题 + 去重后的公共核心题，支持中文、English 和中英对照。'}</p></div>
      <div className="card p-5"><p className="text-sm font-bold text-teal-700">官方资料</p><p className="mt-2 text-xl font-black text-slate-950">Driver Manual</p><p className="mt-2 text-sm leading-6 text-slate-600">{isNJ ? 'New Jersey MVC 提供 Driver Manual 与 Knowledge Test 官方说明。' : 'California DMV 官方提供 Driver Handbook 和 Class C 样题。'}</p></div>
    ...
    <section className="bg-[#f4f7fb] py-10"><div className="page-shell grid gap-5 lg:grid-cols-[1fr_0.8fr]">
      <div className="card p-5"><h2 className="text-2xl font-black text-slate-950">{state.nameZh} 考试准备步骤</h2><div className="mt-4 grid gap-3">{(isCalifornia ? ['先阅读 California DMV 官方 Driver’s Handbook；官方提供中文版本。','使用本站题库熟悉路权、车道线、交通标志、行人、自行车和安全驾驶，可切换中文、English 或中英对照。','完成多次模拟考试，把答错题目集中到错题本反复复习。','申请或预约前再次到 California DMV 官方页面核对证件、费用、考试方式和最新规定。','通过知识考试取得相应许可后，再按年龄和申请类型准备驾驶训练及路考。'] : state.guide).map((item,index)=><div key={item} className="flex gap-3 rounded-md bg-slate-50 p-3"><span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-blue-700 text-sm font-black text-white">{index+1}</span><p className="text-sm leading-6 text-slate-700">{item}</p></div>)}</div><Link href={`/${state.slug}/guide`} className="focus-ring mt-5 inline-flex rounded-md border border-slate-300 px-4 py-2 text-sm font-bold text-slate-800">查看完整考试指南</Link></div>
