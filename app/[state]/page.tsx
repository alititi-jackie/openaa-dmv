...
import WashingtonRules from '@/components/WashingtonRules'
  ...
  const title = state.slug === 'california'
    ...
    : isPennsylvania
      ...
      : isMassachusetts
        ...
        : isWashington
          ? '2026 华盛顿州 DOL 中文题库｜40题模拟考试与驾照笔试练习'
          : stateTitle(state)
  const description = state.slug === 'california'
    ...
    : isPennsylvania
      ...
      : isMassachusetts
        ...
        : isWashington
          ? 'Washington DOL Driving Knowledge Exam 中文学习入口，提供华州专属考点、中文 / English / 中英对照题库、40题模拟考试，并按答对32题通过进行练习。'
...
export default async function StatePage({ params }: Props) {
  ...
  const isMassachusetts = state.slug === 'massachusetts'
  const isWashington = state.slug === 'washington'
  const isCompleteState = isCalifornia || isNJ || isPennsylvania || isMassachusetts || isWashington
  ...
  const pageTitle = isCalifornia ? '2026 加州 DMV 中英双语题库' : isPennsylvania ? '2026 宾州 PennDOT 中文题库' : isMassachusetts ? '2026 麻州 Massachusetts RMV 中英双语题库' : isWashington ? '2026 华盛顿州 Washington DOL 中英双语题库' : stateTitle(state)
  const pageDescription = isCalifornia
    ...
    : isPennsylvania
      ...
      : isMassachusetts
        ...
        : isWashington
          ? `当前提供 ${questions.length} 道华州专属考点与公共核心练习题，支持中文、English 和中英对照，并配套 DOL 40题 / 32题通过模拟考试。`
          : stateDescription(state)
  ...
  return <>
    ...
    {isCompleteState ? <section className="bg-white py-10"><div className="page-shell grid gap-4 md:grid-cols-3">
      <div className="card p-5"><p className="text-sm font-bold text-teal-700">当前题库</p><p className="mt-2 text-3xl font-black text-slate-950">{questions.length} 题</p><p className="mt-2 text-sm leading-6 text-slate-600">{isNJ ? '新泽西专属考点 + 去重筛选后的公共核心题，支持中文、English 和中英对照。' : isPennsylvania ? '宾州专属考试题 + 去重后的公共核心题；说明性内容不混入考题。' : isMassachusetts ? '麻州专属考试题 + 去重后的公共核心题，专属题从第一版起即支持中文、English 和中英对照。' : isWashington ? '华州专属考试题 + 去重后的公共核心题，专属题从第一版起即支持中文、English 和中英对照。' : '加州专属题 + 去重后的公共核心题，支持中文、English 和中英对照。'}</p></div>
      <div className="card p-5"><p className="text-sm font-bold text-teal-700">官方资料</p><p className="mt-2 text-xl font-black text-slate-950">Driver Manual</p><p className="mt-2 text-sm leading-6 text-slate-600">{isNJ ? 'New Jersey MVC 提供 Driver Manual 与 Knowledge Test 官方说明。' : isPennsylvania ? 'PennDOT Driver’s Manual 是正式考试规则与考点的最终依据。' : isMassachusetts ? 'Massachusetts RMV 官方提供 English、简体中文和繁体中文 Class D Driver’s Manual。' : isWashington ? 'Washington DOL 官方 Driver Guide 提供多语言版本，并提供简体中文和繁体中文考试语言。' : 'California DMV 官方提供 Driver Handbook 和 Class C 样题。'}</p></div>
