...
export default async function StatePage({ params }: Props) {
  ...
  if (state.status === 'coming-soon') {
    return <section className="bg-white py-16"><div className="page-shell"><BackLink href="/" label="返回美国 DMV 首页" /><div className="card mt-5 p-6"><p className="text-sm font-bold text-amber-800">题库准备中</p><h1 className="mt-2 text-3xl font-black text-slate-950">{state.nameZh} DMV 驾照题库</h1><p className="mt-3 text-sm leading-6 text-slate-600">{state.summary}</p><div className="mt-5 flex flex-wrap gap-3"><ExternalLinkAnchor href={state.officialUrl} className="bg-slate-950 px-4 py-2 text-sm font-black text-white">打开官方 DMV</ExternalLinkAnchor><Link href="/" className="focus-ring rounded-md border border-slate-300 px-4 py-2 text-sm font-bold text-slate-700">返回选州</Link></div></div></div></section>
  }
  ...
  const faq = isCalifornia ? [
    ...
    { question: '宾州 DMV 驾照题库可以代替官方 Driver’s Manual 吗？', answer: '不可以。本站用于中文、English 和中英对照学习与模拟练习，正式法规与考试要求仍应以 PennDOT 官方 Driver’s Manual 为准。' },
  ...
  return <>
    ...
    {isCompleteState ? <section className="bg-white py-10"><div className="page-shell grid gap-4 md:grid-cols-3">
      <div className="card p-5"><p className="text-sm font-bold text-teal-700">当前题库</p><p className="mt-2 text-3xl font-black text-slate-950">{questions.length} 题</p><p className="mt-2 text-sm leading-6 text-slate-600">覆盖本州道路规则、交通标志、安全驾驶和常见考试知识点；支持的语言可在题库和练习页面直接切换。</p></div>
