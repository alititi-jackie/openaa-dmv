...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!state) return {}
  return { title: `${state.nameZh} DMV 错题本`, description: stateDescription(state), alternates: { canonical: `/${state.slug}/wrong-questions` } }
}
...
export default async function WrongQuestionsPage({ params }: Props) {
  ...
  const state = getLiveStateBySlug(stateSlug)
  if (!state) notFound()
  return <section className="bg-[#f4f7fb] py-10"><JsonLd data={webPageJsonLd(`${state.nameZh} DMV 错题本`, stateDescription(state), `/${state.slug}/wrong-questions`)} /><JsonLd data={breadcrumbJsonLd([{ name: '首页', path: '/' }, { name: state.nameZh, path: `/${state.slug}` }, { name: '错题本', path: `/${state.slug}/wrong-questions` }])} /><div className="page-shell"><BackLink href={`/${state.slug}`} label={`返回${state.nameZh}${state.slug === 'new-jersey' ? ' MVC' : ' DMV'}`} /><div className="mb-6"><p className="text-sm font-bold text-teal-700">Wrong Questions</p><h1 className="mt-2 text-3xl font-black text-slate-950">{state.nameZh} DMV 错题本</h1></div><WrongQuestionsClient questions={getStateQuestions(state.slug)} stateSlug={state.slug} storageKey={`openaa-dmv:${state.slug}:wrong`} /></div></section>
}
