...
import { validateSharedCoreBank } from './shared-core-audit-final'
const sharedCoreBase=sharedCoreQuestions.filter(q=>!/^shared-core-\d+b$/.test(q.id));const sharedCoreBank=validateSharedCoreBank([...sharedCoreBase,...sharedCoreReplacements,...sharedCoreSupplement]);
...
function norm(v:string){return v.toLowerCase().replace(/[\s，。！？、,.!?;；:'"“”‘’（）()\-]/g,'')}function hash(v:string){let h=2166136261;for(let i=0;i<v.length;i++){h^=v.charCodeAt(i);h=Math.imul(h,16777619)}return Math.abs(h>>>0)}function override(q:DmvQuestion){const o=californiaQualityOverrides[q.id];return o?{...q,...o}:q}function balance(q:DmvQuestion){if(q.choices.length<2)return q;const t=hash(q.id)%q.choices.length;if(t===q.answerIndex)return q;const c=[...q.choices],correct=c[q.answerIndex];c.splice(q.answerIndex,1);c.splice(t,0,correct);return{...q,choices:c,answerIndex:t}}function prepare(qs:DmvQuestion[]){const ids=new Set<string>(),texts=new Set<string>();return qs.map(override).filter(q=>{const t=norm(q.question);if(ids.has(q.id)||texts.has(t))return false;ids.add(q.id);texts.add(t);return true}).map(balance)}
export function getQuestionSourceForLanguage(id:string):DmvQuestion|null{const q=[...californiaQuestions,...californiaExpandedQuestions,...sharedCoreBank].find(x=>x.id===id);return q?override(q):null}export function getQuestionsForState(slug:string):DmvQuestion[]{if(!getLiveStateBySlug(slug))return[];return slug==='california'?prepare([...californiaQuestions,...californiaExpandedQuestions,...sharedCoreBank]):prepare(sharedCoreBank)}export function getQuestionsByCategory(slug:string,category:DmvQuestion['category']){return getQuestionsForState(slug).filter(q=>q.category===category)}export function getQuestionCountForState(slug:string){return getQuestionsForState(slug).length}
