...
export function getStateQuestions(stateSlug:string):DmvQuestion[]{
  ...
  // sharedCoreBank is already production-audited as exactly 150 valid exam-study questions.
  // Do not run the broad process/info regex against it again: that removed valid driving-law
  // questions whose explanations happened to contain words such as "license" and collapsed
  // Florida's full bank to roughly one exam's worth of questions.
  if(stateSlug==='florida') return dedupe([...floridaQuestions.filter(isExamQuestion),...floridaSupplementQuestions.filter(isExamQuestion),...sharedCoreBank])
  return shared
}
