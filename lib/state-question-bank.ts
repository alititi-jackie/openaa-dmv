...
import { sharedQuestions } from './dmv-data'
...
export function getStateQuestions(stateSlug:string):DmvQuestion[]{ const shared=getBaseQuestionsForState(stateSlug); if(stateSlug==='new-jersey') return dedupe([...newJerseyQuestions,...newJerseySignQuestions,...shared.filter(isExamQuestion)]); if(stateSlug==='pennsylvania') return dedupe([...pennsylvaniaQuestions,...shared.filter(isExamQuestion)]); if(stateSlug==='massachusetts') return dedupe([...massachusettsQuestions,...massachusettsJolQuestions,...shared.filter(isExamQuestion)]); if(stateSlug==='washington') return dedupe([...washingtonQuestions,...shared.filter(isExamQuestion)]); if(stateSlug==='texas') return dedupe([...texasQuestions,...texasSpeedQuestions,...shared.filter(isExamQuestion)]); if(stateSlug==='florida') return dedupe([...floridaQuestions,...sharedQuestions.filter(isExamQuestion)]); return shared }
export function getStateQuestionsByCategory(stateSlug:string,category:DmvQuestion['category']){ return getStateQuestions(stateSlug).filter((q)=>q.category===category) }
export function getStateQuestionCount(stateSlug:string){ return getStateQuestions(stateSlug).length }
